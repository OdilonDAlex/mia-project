...
threeDots.forEach(function (anchor) {
    anchor.addEventListener('click',(e) => {
        ...
        let postViewContainer = parentDots.parentElement.parentElement;
        let postId = postViewContainer.querySelector('form').elements.post_id.value;
        ...
        if(thisPopUp) {
            ...
            containerForm.innerHTML += `
                    ...
                    <form class="share" action="" method="POST">
            ...
            postViewContainer.querySelector(
                '.pop-up form.delete'
                )
                .addEventListener('submit', (event_) => {
                    event_.preventDefault();
                    axios.post(
                        `${window.origin}/blog/posts/delete`,
                        {
                            post_id: postId,
                        },
                    )
                    .then((result) => {
                        postViewContainer.parentElement.removeChild(postViewContainer);
                    })
                    .catch((error) => {
                        if(error.status == 401){
